@php
    ...
    $defaultAccountsRoute = route('admin.accounts.users');
    $defaultActivityRoute = route('admin.activity.logins');
...
@if($isSadm || $perms?->can_sadm_modify_accountlist)
<div id="admin-accounts-id" class="button-section-container {{ (request()->routeIs('admin.accounts.*') && !$enableTopTabs) ? 'show' : '' }}">
    <div class="button-container {{ request()->routeIs('admin.accounts.*') ? 'force-active' : '' }}" 
         onclick="{{ $enableTopTabs ? "proccedto('{$defaultAccountsRoute}')" : "showButtonSection('admin-accounts-id')" }}">
        <div class="button-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none">
...
{{-- 3. Activity Logs Section (Tree Structured Hierarchy) --}}
@if($isSadm || $perms?->can_access_activity_logs)
<div id="admin-activity-id" class="button-section-container {{ (request()->routeIs('admin.activity.*') && !$enableTopTabs) ? 'show' : '' }}">
    <div class="button-container {{ request()->routeIs('admin.activity.*') ? 'force-active' : '' }}" 
         onclick="{{ $enableTopTabs ? "proccedto('{$defaultActivityRoute}')" : "showButtonSection('admin-activity-id')" }}">
        <div class="button-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none">
    ...
    <div class="functions-container">
        {{-- Group A: Systems --}}
        <div class="nav-tree-group" data-group-name="Systems">
            <div class="nav-tree-title">
                <i class="fa-solid fa-server"></i> Systems
            </div>
        ...
        {{-- Group B: DTS --}}
        <div class="nav-tree-group" data-group-name="DTS">
            <div class="nav-tree-title">
                <i class="fa-solid fa-file-lines"></i> DTS
            </div>
            <div class="function-button {{ request()->routeIs('admin.activity.dts.transaction-logs') ? 'force-active' : '' }}" onclick="proccedto('{{ route('admin.activity.dts.transaction-logs') }}')">Transactions Logs</div>
            <div class="function-button {{ request()->routeIs('admin.activity.dts.update-logs') ? 'force-active' : '' }}" onclick="proccedto('{{ route('admin.activity.dts.update-logs') }}')">Update Logs</div>
            <div class="function-button {{ request()->routeIs('admin.activity.dts.flow-logs') ? 'force-active' : '' }}" onclick="proccedto('{{ route('admin.activity.dts.flow-logs') }}')">Flows Logs</div>
        </div>

        {{-- Group C: RDP --}}
        <div class="nav-tree-group" data-group-name="RDP">
            <div class="nav-tree-title">
                <i class="fa-solid fa-box-archive"></i> RDP
            </div>
            <div class="function-button {{ request()->routeIs('admin.activity.rdp.records-logs') ? 'force-active' : '' }}" onclick="proccedto('{{ route('admin.activity.rdp.records-logs') }}')">Records Logs</div>
            <div class="function-button {{ request()->routeIs(['admin.activity.rdp.volume-conversion-logs', 'admin.activity.rdp.update-logs']) ? 'force-active' : '' }}" onclick="proccedto('{{ route('admin.activity.rdp.volume-conversion-logs') }}')">Volume Conversion Logs</div>
        ...
        <div class="nav-tree-group" data-group-name="Chatify">
            ...
            <div class="function-button {{ request()->routeIs('admin.activity.chat-audit') ? 'force-active' : '' }}" onclick="proccedto('{{ route('admin.activity.chat-audit') }}')">Audit Trail</div>
        </div>
    </div>
</div>
@endif

{{-- 4. Subsystems Section --}}
@if($isSadm || $perms?->can_access_subsystems)
<div id="admin-subsystems-id" class="button-section-container {{ (request()->routeIs('admin.subsystems.*') && !$enableTopTabs) ? 'show' : '' }}">
    <div class="button-container {{ request()->routeIs('admin.subsystems.*') ? 'force-active' : '' }}" 
         onclick="{{ $enableTopTabs ? "proccedto('{$defaultSubsystemsRoute}')" : "showButtonSection('admin-subsystems-id')" }}">
        <div class="button-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none">
...
{{-- 5. Document Tracking Systems Section --}}
@if($isSadm || $perms?->can_access_dts_admin)
<div id="admin-dts-logs-id" class="button-section-container {{ (request()->routeIs('admin.dts.*') && !$enableTopTabs) ? 'show' : '' }}">
    <div class="button-container {{ request()->routeIs('admin.dts.*') ? 'force-active' : '' }}" 
         onclick="{{ $enableTopTabs ? "proccedto('{$defaultDtsRoute}')" : "showButtonSection('admin-dts-logs-id')" }}">
        <div class="button-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none">
...
{{-- 6. Records Disposition Program Section --}}
@if($isSadm || $perms?->can_access_rdp_admin)
<div id="admin-rdp-id" class="button-section-container {{ (request()->routeIs('admin.rdp.*') && !$enableTopTabs) ? 'show' : '' }}">
    <div class="button-container {{ request()->routeIs('admin.rdp.*') ? 'force-active' : '' }}" 
         onclick="{{ $enableTopTabs ? "proccedto('{$defaultRdpRoute}')" : "showButtonSection('admin-rdp-id')" }}">
        <div class="button-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="29" height="29" viewBox="0 0 29 29" fill="none">
...
{{-- 9. Recycle Bin Section --}}
@if($isSadm || $perms?->can_access_recycle_bin)
