...
new #[Layout('layouts.dts')] #[Title('Document Tracking System - Create Internal Transaction')] class extends Component {
    ...
    public function updateRequestor(): void
    {
        ...
        if (!$req || (!$isSuperAdmin && $req->office !== $userOfficeCode)) {
            $this->addError('editRequestorName', 'Unauthorized: Only your office can edit this requestor record.');
            return;
        }
        ...
        if (isset($this->requestor_name) && $this->requestor_name === $this->editRequestorOriginalName) {
            $this->requestor_name = $name;
    ...
    public function save()
    {
        ...
        try {
            ...
            // Insert into dts_transaction_details with requestor_id and source_office = null
...
<div class="rms-container">
    ...
    <form class="rms-form" wire:submit.prevent="save">
                ...
                <div class="form-row">
                    <div class="form-col small-input" style="position: relative;">
                        @php
                            ...
                            $selectedReqRec = (!empty($requestor_name) && !empty($unit_college))
                                ? \DB::table('dts_requestor_history')->where('office', $unit_college)->where('requestor_name', $requestor_name)->first()
                                : null;
            ...
            <div class="actions-row" style="display: flex; gap: 12px; align-items: center; margin-top: 24px; clear: both;">
                ...
                @endif
                @php
                    $emailAccessRequired = DB::table(\Illuminate\Support\Facades\Schema::hasTable('sys_system_settings') ? 'sys_system_settings' : 'system_settings')->where('key', 'dts_email_access_required_internal')->value('value') === 'true';
            ...
            <div style="background: #ffffff; border-radius: 16px; width: 100%; max-width: 440px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); display: flex; flex-direction: column; overflow: hidden;">
                ...
                <div style="padding: 24px; display: flex; flex-direction: column; gap: 16px;">
                    <div style="background: #f0f9ff; border: 1px solid #bae6fd; padding: 10px 12px; border-radius: 8px; font-size: 12px; color: #0369a1; display: flex; align-items: center; gap: 8px;">
                        ...
                        <span>Only your office (<strong>{{ $this->unit_college ?: (auth()->user()?->details?->office?->office_code) }}</strong>) can edit its requestors.</span>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="editRequestorPosition" placeholder="e.g. Department Head" style="width: 100%; padding: 10px 12px; border: 1.5px solid #cbd5e1; border-radius: 8px; font-size: 13px; margin-top: 4px;">
