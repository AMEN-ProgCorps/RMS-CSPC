...
new #[Layout('layouts.dts')] #[Title('External Transactions History - Document Tracking System')] class extends Component {
    use WithPagination;
...
<div>
    ...
    <div style="background: #ffffff; padding: 20px 24px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        <h1 style="font-size: 1.4rem; font-weight: 700; color: #1e293b; margin: 0; display: flex; align-items: center; gap: 8px;">
            <span>📜 Transaction History - External Transactions</span>
        </h1>
        <p style="color: #64748b; font-size: 0.875rem; margin: 4px 0 0 0;">
            Record of external transactions originating from other offices that passed through (received/forwarded) by your office.
        </p>
    </div>
    ...
    @if($historyList->count() === 0)
        <div style="background: #ffffff; border-radius: 12px; padding: 48px; text-align: center; color: #64748b; box-shadow: 0 2px 8px rgba(0,0,0,0.03);">
            ...
            <h3 style="font-size: 1.1rem; font-weight: 600; color: #334155; margin-bottom: 4px;">No External History Records</h3>
            <p style="font-size: 0.875rem; margin: 0;">No external transactions from other offices have passed through your office yet.</p>
        </div>
    @else
        @if($layoutMode === 'table')
