...
new #[Layout('layouts.dts')] #[Title('Internal Transactions History - Document Tracking System')] class extends Component {
    use WithPagination;
    ...
    public function getHistoryTransactionsProperty()
    {
        ...
        // Transactions originating from other offices that passed through (received/forwarded by) user office
...
<div>
    ...
    <div style="background: #ffffff; padding: 20px 24px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        <h1 style="font-size: 1.4rem; font-weight: 700; color: #1e293b; margin: 0; display: flex; align-items: center; gap: 8px;">
            <span>📜 Transaction History - Internal Transactions</span>
        </h1>
        <p style="color: #64748b; font-size: 0.875rem; margin: 4px 0 0 0;">
            Record of internal transactions originating from other offices that passed through (received/forwarded) by your office.
        </p>
    </div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <div style="position: relative; flex: 1; max-width: 400px;">
            <input type="text" wire:model.live.debounce.300ms="searchQuery" placeholder="Search history control no., subject..." style="width: 100%; padding: 10px 14px 10px 38px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem; outline: none;">
    ...
    @php $historyList = $this->historyTransactions; @endphp
    ...
    @if($historyList->count() === 0)
        <div style="background: #ffffff; border-radius: 12px; padding: 48px; text-align: center; color: #64748b; box-shadow: 0 2px 8px rgba(0,0,0,0.03);">
            ...
            <h3 style="font-size: 1.1rem; font-weight: 600; color: #334155; margin-bottom: 4px;">No Internal History Records</h3>
            <p style="font-size: 0.875rem; margin: 0;">No external internal transactions have passed through your office yet.</p>
        </div>
    @else
        @if($layoutMode === 'table')
                    ...
                    <tbody>
                        @foreach($historyList as $t)
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                ...
                                <td style="padding: 14px 16px; color: #0369a1; font-weight: 600;">
                ...
                @foreach($historyList as $t)
                    <div style="background: #ffffff; border-radius: 12px; padding: 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); border: 1px solid #e2e8f0;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                            ...
                            <span style="background: #e2e8f0; color: #334155; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: 600;">{{ ucfirst($t->status) }}</span>
                        </div>
                        ...
                        <div style="font-size: 0.8rem; color: #64748b;">
                            ...
                            <div><strong>Current Office:</strong> {{ $t->current_office_name ?: $t->current_office }}</div>
                        </div>
                    </div>
                @endforeach
            </div>
