                ...
                <form class="receive-card" method="post" action="#" onsubmit="return false;" style="box-shadow: none;">
                    ...
                    @else
                        ...
                        @endif
                            ...
                            <!-- Fixed Tooltip Container (rendered once, positioned via JS) -->
                            <div id="dtsFixedTooltip" style="position: fixed; z-index: 9999; width: 260px; background: #0f172a; color: #f8fafc; padding: 12px 14px; border-radius: 10px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.4), 0 8px 10px -6px rgba(0,0,0,0.2); border: 1px solid #334155; font-family: 'Inter', sans-serif; text-align: left; display: none; pointer-events: none;">
                            </div>

                            <script>
                                document.addEventListener('click', function(e) {
                                    const tooltip = document.getElementById('dtsFixedTooltip');
                                    if (!tooltip) return;
                                    const dot = e.target.closest('.dts-timeline-node-dot');
                                    if (dot) {
                                        const wrapper = dot.closest('.dts-timeline-node-wrapper');
                                        const content = wrapper ? wrapper.querySelector('.dts-node-tooltip-content') : null;
                                        if (content) {
                            ...
                            <!-- Horizontal Progress Line Graph with Click Tooltips -->
                            <div style="width: 100%; overflow-x: auto; padding: 54px 16px 24px 16px; box-sizing: border-box; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; margin-top: 4px; margin-bottom: 16px;">
                                <div style="display: flex; align-items: center; justify-content: space-between; min-width: max-content; padding: 0 30px; position: relative;">
                                    @forelse ($this->visiblePath as $index => $step)
                                        ...
                                        <div class="dts-timeline-node-wrapper">
                                            ...
                                            <!-- Hidden Tooltip Content (read by JS) -->
                                            <div class="dts-node-tooltip-content" style="display: none;">
