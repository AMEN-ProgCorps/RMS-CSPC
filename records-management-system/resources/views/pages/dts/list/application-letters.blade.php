...
new #[Layout('layouts.dts')] #[Title('DTS - Application Letters')] class extends Component {
    ...
    public function openExportModal(): void
    {
        ...
        $available = \App\Helpers\DtsExportHelper::getAvailableColumns('application_letters', 0);
    ...
    {
        $available = \App\Helpers\DtsExportHelper::getAvailableColumns('application_letters', $this->exportFlowCount);
    ...
    public function executeExport()
    {
        ...
        $available = \App\Helpers\DtsExportHelper::resolveAllColumns('application_letters', $this->exportColumns);
        ...
        if ($this->exportFormat === 'excel') {
            $rows = \App\Helpers\DtsExportHelper::fetchExportRecords('application_letters', $filters, $this->selectedIds);
            $filename = 'dts-application-letters-' . now()->format('Y-m-d') . '.xls';
            $meta = [
                'title'       => \App\Helpers\DtsExportHelper::getCategoryTitle('application_letters'),
        ...
        if ($this->activeAction === 'For Revision') {
            ...
            if ($nextSequence) {
                ...
                if ($destOfficeCode === 'ORIGIN') {
                    ...
                    if ($originOffice && $originOffice->cluster) {
                        $cluster = DB::table(\Illuminate\Support\Facades\Schema::hasTable('sys_cluster') ? 'sys_cluster' : 'cluster')->where('cluster_code', $originatedFrom)->first();
                        if ($cluster && $cluster->cluster_head) {
...
<div class="rms-container">
    ...
    <!-- View QR Code Modal -->
    <div id="dts-qr-view-modal" class="modal-backdrop" wire:ignore wire:key="dts-qr-view-modal-apps" style="display: none; z-index: 1000000; align-items: center; justify-content: center;" onclick="closeQrViewModal()">
    ...
    <x-dts.export-modal
        ...
        category="application_letters"
        :available-columns="\App\Helpers\DtsExportHelper::getAvailableColumns('application_letters', $exportFlowCount)"
