...
new #[Layout('layouts.dts')] #[Title('Document Tracking System - List Application Letters')] class extends Component {
    use WithPagination;
            ...
            @forelse ($this->transactions as $index => $t)
                <div class="dts-box-card" style="background: white; border: 1.5px solid #ced4da; border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); display: flex; flex-direction: column; justify-content: space-between; position: relative;">
                    ...
                    <div style="font-size: 13px; color: #4b5563; line-height: 1.6; margin-top: 12px; font-family: Roboto, sans-serif;">
                        ...
                        <div style="margin-bottom: 6px;"><strong>Name of Applicant:</strong> {{ $t->requestor_name ?? 'N/A' }}</div>
                        <div style="margin-bottom: 6px;"><strong>Position:</strong> {{ $t->classification ?? 'N/A' }}</div>
                        ...
                        <div style="margin-bottom: 14px;"><strong>Type of Document:</strong> {{ $t->document_name ?? 'Application Letter' }}</div>

                        <div style="margin-bottom: 6px;"><strong>Receive From:</strong> <span style="color: #ef4444; font-weight: 500;">{{ $t->from_office }}</span></div>
                        <div style="margin-bottom: 14px;"><strong>Receive Date:</strong> {{ $t->date_time_received ? \Carbon\Carbon::parse($t->date_time_received)->format('Y-m-d H:i') : 'N/A' }}</div>

                        <div style="margin-bottom: 14px;"><strong>Next Receiving Office:</strong> {{ $t->destination }}</div>
                        ...
                        <div style="margin-bottom: 6px;"><strong>Elapsed Day:</strong> <span style="color: #ef4444; font-style: italic;">{{ \Carbon\Carbon::parse($t->date_time_received ?? $t->date_created)->diffInDays(now()) }} day(s) (period that has passed between the received date and the later date)</span></div>
                    </div>
