...
new #[Layout('layouts.dts')] #[Title('DTS - External Transactions')] class extends Component {
    ...
    public function mount(): void
    {
        ...
        if ($perms && !$perms->is_sadm && !$perms->can_dts_use_external) {
            abort(403, 'Unauthorized access to External transactions list.');
        }
    }
    ...
    public function openExportModal(): void
    {
        ...
        $available = \App\Helpers\DtsExportHelper::getAvailableColumns('external', 0);
    ...
    {
        $available = \App\Helpers\DtsExportHelper::getAvailableColumns('external', $this->exportFlowCount);
    ...
    public function executeExport()
    {
        ...
        $available = \App\Helpers\DtsExportHelper::resolveAllColumns('external', $this->exportColumns);
        ...
        if ($this->exportFormat === 'excel') {
            $rows = \App\Helpers\DtsExportHelper::fetchExportRecords('external', $filters, $this->selectedIds);
            $filename = 'dts-external-transactions-' . now()->format('Y-m-d') . '.xls';
            $meta = [
                'title'       => \App\Helpers\DtsExportHelper::getCategoryTitle('external'),
    ...
    public function getTransactionsProperty()
    {
        ...
        $list->getCollection()->transform(function ($t) {
            $latestLog = DB::table((\Illuminate\Support\Facades\Schema::hasTable('dts_transaction_logs') ? 'dts_transaction_logs' : 'sub_document_tracking_system_logs') . ' as log')
...
<div class="rms-container">
                            ...
                            @else
                                @php
                                    ...
                                    if (!empty($flowName) && str_starts_with($flowName, 'Flow for ')) {
                                        $flowName = ucfirst($selectedTransaction->trans_type ?? 'External');
                                    }
                                @endphp
    ...
    <!-- View QR Code Modal -->
    <div id="dts-qr-view-modal" class="modal-backdrop" wire:ignore wire:key="dts-qr-view-modal-external" style="display: none; z-index: 1000000; align-items: center; justify-content: center;" onclick="closeQrViewModal()">
    ...
    <x-dts.export-modal
        ...
        category="external"
        :available-columns="\App\Helpers\DtsExportHelper::getAvailableColumns('external', $exportFlowCount)"
