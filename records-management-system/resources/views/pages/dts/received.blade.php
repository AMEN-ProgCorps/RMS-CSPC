                ...
                if ($tData['is_last_step']) {
                    ...
                    if ($nextCode === '[HUB]') {
                        ...
                        if (count($hubOffices) > 0) {
                            ...
                            $priorLogs = DB::table(\Illuminate\Support\Facades\Schema::hasTable('dts_transaction_logs') ? 'dts_transaction_logs' : 'sub_document_tracking_system_logs')
                                ->where('transaction_id', $transId)
                ...
                $q->where('dt.current_office', $userOfficeCode)
                  ->orWhereExists(function($sub) use ($userOfficeCode, $logsTable) {
                      $sub->select(DB::raw(1))
                          ...
                          ->whereNotNull('sub_logs.date_in')
                          ->where('sub_logs.type', 'received');
                  });
            });
