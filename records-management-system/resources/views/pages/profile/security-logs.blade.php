            ...
            <table class="table table-striped">
                ...
                <tbody>
                    @forelse ($securityLogs as $log)
                        <tr>
                            <td>
                                <span class="badge" style="padding: 4px 10px; border-radius: 99px; font-size: 11px; font-weight: bold; text-transform: uppercase; background-color: {{ stripos($log->status_name, 'success') !== false ? 'rgba(16, 185, 129, 0.15)' : (stripos($log->status_name, 'logout') !== false || stripos($log->status_name, 'timeout') !== false ? 'rgba(245, 158, 11, 0.15)' : 'rgba(239, 68, 68, 0.15)') }}; color: {{ stripos($log->status_name, 'success') !== false ? '#10b981' : (stripos($log->status_name, 'logout') !== false || stripos($log->status_name, 'timeout') !== false ? '#f59e0b' : '#ef4444') }}; border: 1px solid {{ stripos($log->status_name, 'success') !== false ? 'rgba(16, 185, 129, 0.3)' : (stripos($log->status_name, 'logout') !== false || stripos($log->status_name, 'timeout') !== false ? 'rgba(245, 158, 11, 0.3)' : 'rgba(239, 68, 68, 0.3)') }};">
                                    {{ $log->status_name }}
                                </span>
                            </td>
