...
new #[Layout('layouts.rdp')] #[Title('Received Documents - Document Control System')] class extends Component {
    use WithPagination;
    ...
    public function dismissDocument(int $id): void
    {
        ...
        $this->successMessage = 'Controlled document marked as dismissed.';
    ...
    public function importDcsDocument(string $docNo): void
    {
        $this->clearMessages();
        $docNo = trim($docNo);

        if (empty($docNo)) {
            $this->errorMessage = 'Please provide a valid DCS Document Number.';
            return;
        }
        ...
        $existing = DB::table('rdp_received_documents')
            ->where('source_subsystem', 'DCS')
            ->where('document_code', $docNo)
            ->first();

        if ($existing) {
            $this->errorMessage = "DCS Document '{$docNo}' has already been received into RDP.";
            return;
        }

        // Find document in dcs_masterlist_registration
        $dcsDoc = DB::table('dcs_masterlist_registration')
            ->where('doc_no', $docNo)
            ->first();

        if (!$dcsDoc) {
            $this->errorMessage = "DCS Document '{$docNo}' was not found in the Control System masterlist.";
            return;
        }
        ...
        DB::table('rdp_received_documents')->insert([
            'source_subsystem'    => 'DCS',
            'document_code'       => $dcsDoc->doc_no,
            'document_title'      => $dcsDoc->doc_title ?? 'Untitled Controlled Document',
            'description'         => 'Controlled document from DCS Masterlist. Rev: ' . ($dcsDoc->revise_no ?? '0') . ($dcsDoc->keywords ? ' | Keywords: ' . $dcsDoc->keywords : ''),
            'origin_office'       => $dcsDoc->originator_name ?? null,
            'target_office'       => null,
            'date_received'       => $dcsDoc->doc_registered_date ? Carbon::parse($dcsDoc->doc_registered_date)->toDateString() : now()->toDateString(),
            'file_path'           => $dcsDoc->scanned_masterlist ?? null,
            'file_name'           => $dcsDoc->scanned_masterlist_original_name ?? null,
            'document_id_handler' => null,
            'status'              => 'pending',
            'sent_by_user'        => $user?->id,
            'metadata'            => json_encode([
                'doc_type_id'     => $dcsDoc->doc_type_id ?? null,
                'revision_no'     => $dcsDoc->revise_no ?? null,
                'originator'      => $dcsDoc->originator_name ?? null,
                'keywords'        => $dcsDoc->keywords ?? null,
        ...
        $this->successMessage = "Successfully imported DCS Document '{$docNo}' into Received Documents!";
    ...
    public function with(): array
    {
        ...
        // Counters
        $totalAll = DB::table('rdp_received_documents')->where('source_subsystem', 'DCS')->count();
        $totalPending = DB::table('rdp_received_documents')->where('source_subsystem', 'DCS')->where('status', 'pending')->count();
        $totalAppraised = DB::table('rdp_received_documents')->where('source_subsystem', 'DCS')->where('status', 'appraised')->count();
        $totalDismissed = DB::table('rdp_received_documents')->where('source_subsystem', 'DCS')->where('status', 'dismissed')->count();
...
<div class="received-docs-container">
    ...
    <div class="header-section">
        <div class="header-title-box">
            ...
            <div>
                <h1>
                    Received Documents
                    <span class="subsystem-badge-dcs">Document Control System (DCS)</span>
                </h1>
                <p>Official controlled documents and revisions routed from Document Control System for appraisal and archiving.</p>
            </div>
        </div>
        ...
        <div class="action-bar">
            ...
            <div class="search-box">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search doc no, title...">
            </div>
            ...
            <!-- Import / Pull from DCS Button -->
            <button type="button" wire:click="openImportModal" class="btn-import-dcs">
    ...
    @if($documents->isEmpty())
        <div class="table-card empty-state">
            ...
            <h3 style="font-size:16px;font-weight:600;color:#334155;margin:0 0 6px 0;">No controlled documents found</h3>
            <p style="font-size:13px;margin:0 0 16px 0;">No incoming DCS documents match the current filter or search criteria.</p>
            <button type="button" wire:click="openImportModal" class="btn-import-dcs" style="margin:0 auto;display:inline-flex;">
                Pull from Document Control System
            </button>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    @foreach($documents as $doc)
                        <tr>
                            <td>
                                <span class="code-pill-dcs">{{ $doc->document_code }}</span>
                            </td>
                            <td>
                            ...
                            <td>
                                @if(!empty($doc->file_name) || !empty($doc->file_path))
                                    <span style="display:inline-flex;align-items:center;gap:4px;font-size:12px;color:#db2777;">
                                        ...
                                        {{ Str::limit($doc->file_name ?: 'Controlled Doc', 20) }}
                                    </span>
                                @else
            ...
            @foreach($documents as $doc)
                <div class="doc-card">
                    <div>
                        ...
                        <div class="card-meta">
                            <div><strong>Originator:</strong> {{ $doc->origin_office ?: 'Unspecified' }}</div>
                            <div><strong>Registered:</strong> {{ $doc->date_received ? Carbon::parse($doc->date_received)->format('M d, Y') : '—' }}</div>
                            @if(!empty($doc->file_name))
                                <div style="color: #db2777;"><strong>File:</strong> {{ Str::limit($doc->file_name, 25) }}</div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="custom-modal-box">
                ...
                <div class="modal-body">
                    <div class="detail-row">
                        ...
                        <div class="detail-value"><span class="code-pill-dcs">{{ $selectedDoc->document_code }}</span></div>
                    </div>
                    <div class="detail-row">
                    ...
                    <div class="detail-row">
                        <div class="detail-label">Originator / Dept:</div>
                    ...
                    @if($selectedDoc->file_name)
                        <div class="detail-row">
                            ...
                            <div class="detail-value" style="color: #db2777;">
    ...
    <!-- Import from DCS Modal -->
    @if($showImportModal)
        <div class="custom-modal-backdrop" wire:click.self="closeImportModal">
            <div class="custom-modal-box" style="max-width: 680px;">
                <div class="modal-header">
                    <div>
                        <h3 style="margin:0;">Import Document from DCS Masterlist</h3>
                        <p style="font-size:12px;color:#64748b;margin:3px 0 0 0;">Select or search registered controlled documents to receive into RDP.</p>
                    </div>
                ...
                <div class="modal-body">
                    <div style="margin-bottom: 16px;">
                        <input type="text" wire:model.live.debounce.300ms="importSearch" 
                               placeholder="Search document no or title in DCS..." 
                                ...
                                <div style="padding: 12px 14px; border: 1px solid #e2e8f0; border-radius: 8px; display: flex; align-items: center; justify-content: space-between; background: #fafafa;">
                                    <div>
                                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                                            <span class="code-pill-dcs">{{ $dcsItem->doc_no }}</span>
                                            <span style="font-size: 11px; color: #64748b;">Rev: {{ $dcsItem->revise_no ?? '0' }} | {{ $dcsItem->originator_name ?: 'No originator' }}</span>
                                        </div>
                                        <div style="font-size: 13px; font-weight: 600; color: #0f172a;">{{ $dcsItem->doc_title ?: 'No title' }}</div>
                                    </div>
                                    <button type="button" wire:click="importDcsDocument('{{ $dcsItem->doc_no }}')" class="btn-import-dcs" style="padding: 6px 12px; cursor: pointer;">
                                        Receive
                                    </button>
                                </div>
                            @endforeach
